...
| **T19** existence oracle in the ack | an unreachable caller gets `approval_needed` for a missing dir (no disclosure); a whitelisted caller gets `directory_not_known` | unit (real socket) | `control::tests::missing_directory_is_not_disclosed_to_an_unreachable_caller`, `..._is_directory_not_known_for_a_reachable_caller` |
| **T20** non-canonical target | a `..`/trailing-slash/symlink spawn `cwd` is stamped canonical on the queued submission (whitelist key = label = launch dir); an ambiguous whitelist path is refused | unit + A | `control::tests::a_spawn_cwd_is_stamped_canonical_before_routing`, `mailbox::tests::authenticate_canonicalizes_a_directory`, `mailbox::tests::whitelist_refuses_a_path_straddling_the_separator`, e2e-pi step 8b |

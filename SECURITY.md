...
- **corrald** is the single trusted broker for agent-initiated messaging. It
  owns the control socket, the whitelist gate, and the approval surface. Exactly
  one may run (singleton guard). `[in place]`
...
corrald opens the file, resolves its canonical physical path race-free (via the
fd readlink, so a symlink swapped after open cannot redirect it), and derives
`fromCwd` from where the file physically lives. `fromSession` is verified
against the registry: the claimed session's record must physically live in that
same directory. The act of writing inside `D` is the proof of identity `D`,
unforgeable from another box.
...
A consumer resolves the symlink to its canonical physical path and derives `cwd`
from the location, ignoring any `cwd` in the content. A record physically in
`evil/` is attributed to `evil/`, so an attacker cannot claim a directory it
cannot write. The record's own `sessionId` must equal the symlink filename, and
consumers deduplicate by resolved path.
...
**Mitigation `[designed]`:** trust-on-first-use approved command templates.
corrald is the sole approver: it watches the registry, and on a never-seen
`(label, normalized-argv-template)` it asks the operator once, showing the full
argv, then writes the template to `~/.corral/state/approved-commands.json`.
Templates normalize the record's own `sessionId` and `cwd` to placeholders, so
each harness kind is approved once, ever. Viewers only read the store and refuse
to launch an unapproved command. The operator seeing the literal argv at
first-approval is the actual gate; the approval surface must display it.
Depends on T5 (the store must be unwritable by agents).
...
**Mitigation `[designed]`:** a positional rule plus a fenced body. corrald keeps
the leading tag and delivers the body verbatim inside `--- begin message ---`
and `--- end message ---`. The charter and CONVENTION state the rule: the first
`[from …]` line is authentic; anything resembling a tag inside the fence is
data. No escaping, no body rewriting.
...
3. **corrald as a kept singleton.** A systemd user service with
   restart-on-failure keeps the sole broker alive. Command approval (T4) and
   messaging require it; "no daemon means no new approvals" is a deliberate
   property.
...
T6) are open; a compromised sandboxed agent can spoof its identity, plant a
record, tamper with the whitelist, hijack the control socket, or induce
unsandboxed code execution. The `[designed]` mitigations close them and are
specified in the hardening design. Until they land, treat the whitelist and the
gates as a convenience, not a security control.
